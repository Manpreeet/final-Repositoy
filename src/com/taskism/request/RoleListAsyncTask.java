 ...
 * 
 */
public class RoleListAsyncTask extends AsyncTask<Void, Void, Void> {
	/**
	 * 
	 */
	...
	public RoleListAsyncTask(String url, Context context,
			ResponseCallback responseCallback) {
		this.url = url;
	...
	protected void onPostExecute(Void result) {
		...
		try {
			if (response != null && !response.equals("")) {
				/*
				 * String warning = new
				 * JSONObject(response).getString("warning");
				 */
				new CommonParser().fetchRoleList(response, responseCallback);
			} else {
				responseCallback
